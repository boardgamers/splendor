...
describe("buy", () => {
  ...
  it("buys a reserved card", () => {
    ...
    const player = state.players[0]!;
    for (const [color, n] of Object.entries(card.cost)) {
      if (n > 0) player.tokens[color as keyof typeof player.tokens] += n;
    }
    const next = applyMove(state, { action: "buy", cardId }, 0);
  ...
  it("prefers color gems over gold when both can pay", () => {
    ...
    const cardId = state.table[0].find((id) => {
      ...
      return card.points === 0 && Object.values(card.cost).some((n) => n === 3) && Object.values(card.cost).every((n) => n === 0 || n === 3);
    })!;
  ...
  it("does not spend gems beyond the discounted cost", () => {
    ...
    const player = state.players[0]!;
    for (const [color, n] of Object.entries(card.cost)) {
      if (n > 0) player.tokens[color as keyof typeof player.tokens] = n + 1;
    }
    const next = applyMove(state, { action: "buy", cardId }, 0);
    const tokens = next.players[0]!.tokens;
    for (const [color, n] of Object.entries(card.cost)) {
      if (n > 0) assert.equal(tokens[color as keyof typeof tokens], 1);
    }
  });
});
