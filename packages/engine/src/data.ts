...
type Row = [tier: Tier, bonus: GemColor, points: number, white: number, blue: number, green: number, red: number, black: number];
...
export function nobleById(id: number): Noble {
  ...
  if (!noble) throw new Error(`unknown noble id ${id}`);
  return noble;
}
