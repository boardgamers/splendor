...
function currentPlayerState(state: GameState): PlayerState {
  ...
  if (!player || player.dropped) fail("no active current player");
  return player;
}

function assertTurn(state: GameState, playerIndex: number): PlayerState {
  if (state.ended) fail("the game is over");
  if (playerIndex !== state.current) fail(`it is ${playerName(state, state.current)}'s turn`);
...
function doTake(state: GameState, player: PlayerState, move: Extract<Move, { action: "take" }>): void {
  if (move.gems.length !== 3) fail("you must take exactly 3 different gems");
  if (new Set(move.gems).size !== 3) fail("the 3 gems must be of different colors");
  for (const color of move.gems) if (!GEM_COLORS.includes(color)) fail(`${color} is not a gem color (gold cannot be taken directly)`);
  takeGems(state, player, move.gems);
}

function doTake2(state: GameState, player: PlayerState, move: Extract<Move, { action: "take2" }>): void {
  if (!GEM_COLORS.includes(move.color)) fail(`${move.color} is not a gem color (gold cannot be taken directly)`);
...
function doReserve(state: GameState, player: PlayerState, move: Extract<Move, { action: "reserve" }>): void {
  if (player.reserved.length >= MAX_RESERVED) fail(`you may reserve at most ${MAX_RESERVED} cards`);
  if (tokenTotal(player) >= MAX_TOKENS) fail(`you may hold at most ${MAX_TOKENS} gems`);
  let cardId: number;
  if (move.cardId !== undefined) {
    const spot = findTableCard(state, move.cardId);
    if (!spot) fail(`card ${move.cardId} is not available on the table`);
    cardId = move.cardId;
    (state.table[spot.tier - 1] as number[]).splice(spot.index, 1);
    drawReplacement(state, spot.tier);
  } else if (move.tier !== undefined) {
    if (!canReserveFromDeck(state, move.tier)) fail(`tier ${move.tier} deck is empty`);
...
function doBuy(state: GameState, player: PlayerState, move: Extract<Move, { action: "buy" }>): void {
  ...
  if (!spot && reservedIndex < 0) fail(`card ${move.cardId} is neither on the table nor in your reserved cards`);
  const { cost, goldNeeded } = cardCostFor(state, state.current, move.cardId);
  if (goldNeeded > player.tokens.gold) fail(`you cannot afford card ${move.cardId} (${goldNeeded} gold needed, ${player.tokens.gold} owned)`);
...
function doNoble(state: GameState, player: PlayerState, move: Extract<Move, { action: "noble" }>): void {
  ...
  if (index < 0) fail(`noble ${move.nobleId} does not visit you now`);
...
export function endGameCheck(state: GameState): void {
  if (!state.lastRound) {
    const scoring = state.players.map((p, i) => (!p.dropped && prestige(p) >= PRESTIGE_TARGET ? i : -1)).filter((i) => i >= 0);
    if (scoring.length > 0) {
  ...
  if (state.lastRound && state.current === state.lastPlayer) finishGame(state);
}
...
export function finishGame(state: GameState): void {
  ...
  for (let i = 0; i < state.players.length; i++) {
    if (!state.players[i]?.dropped && (finalScores[i] as number) > best) best = finalScores[i] as number;
  }
  const top = activePlayers(state).filter((i) => finalScores[i] === best);
  let fewestCards = Infinity;
  for (const i of top) fewestCards = Math.min(fewestCards, (state.players[i] as PlayerState).cards.length);
...
function postAction(state: GameState): void {
  endGameCheck(state);
  if (!state.ended) advanceTurn(state);
}
...
function validate(state: GameState, move: Move, playerIndex: number): void {
  ...
  if (state.pendingNobles.length > 0) {
    if (move.action !== "noble") fail("a noble is waiting to visit you — choose one");
    if (!state.pendingNobles.includes(move.nobleId)) fail(`noble ${move.nobleId} does not visit you now`);
    return;
  }
  if (move.action === "noble") fail("no noble visits you this turn");
  switch (move.action) {
    case "take": {
      if (move.gems.length !== 3) fail("you must take exactly 3 different gems");
      if (new Set(move.gems).size !== 3) fail("the 3 gems must be of different colors");
      for (const color of move.gems) {
        if (!GEM_COLORS.includes(color)) fail(`${color} is not a gem color (gold cannot be taken directly)`);
        if (state.bank[color] <= 0) fail(`the bank has no ${color} left`);
      }
      if (tokenTotal(player) + 3 > MAX_TOKENS) fail(`you may hold at most ${MAX_TOKENS} gems (you have ${tokenTotal(player)})`);
      return;
    }
    case "take2": {
      if (!GEM_COLORS.includes(move.color)) fail(`${move.color} is not a gem color (gold cannot be taken directly)`);
      if (state.bank[move.color] < TAKE2_MIN_BANK) fail(`you can take 2 ${move.color} only if the bank has at least ${TAKE2_MIN_BANK}`);
      if (tokenTotal(player) + 2 > MAX_TOKENS) fail(`you may hold at most ${MAX_TOKENS} gems (you have ${tokenTotal(player)})`);
      return;
    }
    case "reserve": {
      if (player.reserved.length >= MAX_RESERVED) fail(`you may reserve at most ${MAX_RESERVED} cards`);
      if (tokenTotal(player) >= MAX_TOKENS) fail(`you may hold at most ${MAX_TOKENS} gems`);
      if (move.cardId !== undefined) {
        if (!findTableCard(state, move.cardId)) fail(`card ${move.cardId} is not available on the table`);
      } else if (move.tier !== undefined) {
        if (!canReserveFromDeck(state, move.tier)) fail(`tier ${move.tier} deck is empty`);
      } else {
        fail("reserve needs a cardId (from the table) or a tier (from a deck)");
...
export function dropPlayer(state: GameState, playerIndex: number): GameState {
  ...
  if (!player || player.dropped) fail(`player ${playerIndex} is not in the game`);
  player.dropped = true;
  ...
  if (active.length <= 1) {
    ...
    state.messages.push(winner !== null ? `${playerName(state, winner)} wins — all opponents left` : "Game over — no players left");
    return state;
  }
