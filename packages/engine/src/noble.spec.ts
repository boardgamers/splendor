...
describe("nobles", () => {
  ...
  it("requires a choice when several nobles are eligible", () => {
    ...
    giveBonuses(state, 0, Object.fromEntries(Object.entries(noble333.requirement).filter(([, v]) => v === 3)) as Partial<Record<GemColor, number>>);
    ...
    assert.throws(
      () => applyMove(next, { action: "take2", color: "ruby" }, 0),
      /noble/
    );
    ...
    assert.deepEqual(pendingNoblesFor(visited, 0), [noble44.id], "the player still qualifies for the other noble next turn");
  });
