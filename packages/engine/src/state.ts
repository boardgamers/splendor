...
export function setup(playerCount: number, options: Record<string, unknown>, seed: string): GameState {
  ...
  const decks = ([1, 2, 3] as Tier[]).map((tier) => shuffle(CARDS.filter((c) => c.tier === tier).map((c) => c.id), rand));
...
export function bonuses(player: PlayerState): Record<GemColor, number> {
  ...
  for (const id of player.cards) result[cardById(id).bonus]++;
  return result;
}

export function prestige(player: PlayerState): number {
  let total = 0;
  for (const id of player.cards) total += cardById(id).points;
  for (const id of player.nobles) total += nobleById(id).points;
  return total;
}
...
export function cardCostFor(state: GameState, playerIndex: number, cardId: number): { cost: Record<GemColor, number>; goldNeeded: number } {
  const player = state.players[playerIndex];
  if (!player) throw new Error(`unknown player ${playerIndex}`);
...
export function findTableCard(state: GameState, cardId: number): { tier: Tier; index: number } | null {
  for (let t = 0; t < 3; t++) {
    ...
    if (index >= 0) return { tier: (t + 1) as Tier, index };
  }
  return null;
}
...
export function nobleEligible(state: GameState, playerIndex: number, nobleId: number): boolean {
  if (!state.nobles.includes(nobleId)) return false;
...
export function availableMoves(state: GameState, playerIndex?: number): string[] {
  ...
  if (state.pendingNobles.length > 0) return state.pendingNobles.map((id) => `noble:${id}`);
  ...
  if (total < MAX_TOKENS) {
    ...
    for (const color of GEM_COLORS) {
      if (state.bank[color] >= TAKE2_MIN_BANK && total + 2 <= MAX_TOKENS) moves.push(`take2:${color}`);
    }
  }
  if (player.reserved.length < MAX_RESERVED && total < MAX_TOKENS) {
    for (const row of state.table) for (const id of row) moves.push(`reserve:${id}`);
    for (const tier of [1, 2, 3] as Tier[]) if (canReserveFromDeck(state, tier)) moves.push(`reserve-deck:${tier}`);
  }
  for (const row of state.table) for (const id of row) if (canBuy(state, index, id)) moves.push(`buy:${id}`);
  for (const id of player.reserved) if (canBuy(state, index, id)) moves.push(`buy:${id}`);
  return moves;
}
