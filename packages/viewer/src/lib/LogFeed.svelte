...
<div class="side">
  <div class="prefs">
    <label title="Smaller development cards">
      <input type="checkbox" checked={compact} onchange={(e) => store.updatePreference("compactCards", e.currentTarget.checked)} />
      Compact cards
    </label>
    <label title="Show the event feed below the board">
      <input type="checkbox" checked={showFeed} onchange={(e) => store.updatePreference("logFeed", e.currentTarget.checked)} />
      Event feed
    </label>
  </div>
...
<style>
  ...
  .prefs input { accent-color: var(--gold); }
  .feed {
