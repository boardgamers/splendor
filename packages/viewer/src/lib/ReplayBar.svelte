<script lang="ts">
  ...
  const moves = $derived.by(() => {
    ...
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i] ?? "";
      if (line.startsWith("Game started")) continue;
      count++;
...
{#if replay.active}
  <div class="replaybar">
    ...
    <div class="track">
      ...
      <button class="nav" disabled={replay.current >= replay.end} onclick={() => store.replayToEntry(replay.end)} title="Live">⏭</button>
    </div>
  </div>
{/if}
...
<style>
  ...
  .pos { color: var(--text-dim); font-size: 12px; }
  .track {
    display: flex;
    align-items: center;
    gap: 8px;
