...
export class ViewerStore {
  ...
  private replayTo(to: number): void {
    const live = this.liveState;
    if (!live || !this.replay.active) return;
  ...
  get takeDraft(): Move | null {
    ...
    if (pick.length === 3) return { action: "take", gems: pick };
    return null;
  }

  gemEnabled(color: GemColor): boolean {
    const s = this.liveState;
    if (!s || !this.myTurn || s.bank[color] <= 0) return false;
    const pick = this.gemPick;
    if (pick.length === 0) {
    ...
    if (pick.length === 1) {
      if (color === first) return s.bank[color] >= 4 && this.myTokens + 2 <= 10;
  ...
  get takeHint(): string {
    ...
    if (pick.length === 2 && pick[0] === pick[1]) return `Take 2 ${pick[0]}?`;
    if (pick.length === 2) return "Pick a third, different gem";
  ...
  pickGem(color: GemColor): void {
    ...
    if (this.reserving) this.reserving = false;
    const pick = this.gemPick;
  ...
  toggleReserve(): void {
    ...
    if (this.reserving) this.gemPick = [];
  }
  ...
  clickCard(cardId: number, zone: "table" | "reserved"): void {
    ...
    if (zone === "reserved" && !this.isOwnReserved(cardId)) return;
    if (this.affordable(cardId)) this.send({ action: "buy", cardId });
  }

  clickDeck(tier: Tier): void {
    if (!this.myTurn || !this.reserving || !this.canReserveAtAll()) return;
